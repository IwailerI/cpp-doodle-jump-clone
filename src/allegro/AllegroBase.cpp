...
AllegroBase::AllegroBase()
    : alDisplay_(0), alEventQueue_(0), alTimer_(0), exit_(false) {
    memset(pressedKeys_, 0, sizeof(pressedKeys_));
}

AllegroBase::~AllegroBase() { Destroy(); }
...
bool AllegroBase::Init(int screenWidth, int screenHeight, int fps) {
    ...
    al_register_event_source(alEventQueue_,
                             al_get_display_event_source(alDisplay_));
    al_register_event_source(alEventQueue_,
                             al_get_timer_event_source(alTimer_));
...
void AllegroBase::Exit() { exit_ = true; }

bool AllegroBase::IsPressed(int keycode) { return pressedKeys_[keycode]; }
